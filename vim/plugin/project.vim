...
function! s:ProjectLoad()
	...
	" modified by pqz
	" read session and viminfo
	if filereadable(proj_data . '/project.session')
		exe 'source ' . proj_data . '/project.session'
	endif

	if filereadable(proj_data . '/project.viminfo')
		exe 'rviminfo ' . proj_data . '/project.viminfo'
	endif
...
function! s:ProjectQuit()
	...
	" modifyed by pqz
	exe 'mksession!' . proj_data . '/project.session'
	exe 'wviminfo!' . proj_data . '/project.viminfo'
